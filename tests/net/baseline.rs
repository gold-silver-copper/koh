...
#[test]
fn the_fault_link_is_the_only_path() {
...
async fn measure(profile: Profile, outage: bool, seed: u64) -> anyhow::Result<Metrics> {
    ...
    for n in 1..=KEYSTROKES {
        ...
        let painted = client
            .wait_until(long, |t| t.matches('x').count() >= base.saturating_add(n))
            .await
    ...
    let recovery = if outage {
        ...
        let ends = Instant::now()
            .checked_add(outage_len)
            .context("the outage ends beyond the clock's range")?;
    ...
    let percentile = |p: usize| {
        echoes
            .get(KEYSTROKES.saturating_mul(p).div_euclid(100))
            .copied()
...
fn baseline() {
    crate::harness::runtime().expect("tokio runtime").block_on(async {
