...
| `stress-evil-peer` *(needs the cross-compiled evil-peer; else self-SKIPs)* | the **malicious-peer harness**: crafted stream/flood attacks (an over-cap message prefix, input floods, resize/keys floods, garbage, a wrong ALPN) + an admission-stall from a malicious **client**, and bad-admit/stall-admit from a malicious **server** | the server stays **alive + memory-bounded + panic-free** under every client attack (witness session intact, fresh client still connects), bounds a stalled admission, and a real koh **client refuses** a server that sends a bad admission byte or never admits (never reaches "connected.") |
