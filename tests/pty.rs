...
async fn spawns_and_streams_output() {
    ...
    let (mut pty, mut rx) =
        Pty::spawn(24, 80, Some("echo"), None, "xterm-256color").expect("spawn echo");
...
async fn interactive_shell_echoes_input() {
    ...
    let (mut pty, mut rx) = Pty::spawn(24, 80, None, None, "xterm-256color").expect("spawn shell");
...
async fn write_input_takes_shared_ref_and_preserves_order() {
    ...
    let (pty, mut rx) = Pty::spawn(24, 80, None, None, "xterm-256color").expect("spawn shell");
    tokio::time::sleep(Duration::from_millis(300)).await;
...
async fn dropping_pty_eofs_child_and_stops_writer() {
    ...
    let (pty, mut rx) = Pty::spawn(24, 80, Some("cat"), None, "xterm-256color").expect("spawn cat");
...
async fn shutdown_joins_both_io_threads_without_deadlock() {
    ...
    let (pty, mut rx) =
        Pty::spawn(24, 80, Some("sh"), None, "xterm-256color").expect("spawn shell");
