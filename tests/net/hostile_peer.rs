...
async fn an_oversized_client_message_closes_the_connection_only() -> anyhow::Result<()> {
    let net = net();
    let evil = identity();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn a_client_cannot_open_a_second_stream() -> anyhow::Result<()> {
    let net = net();
    let evil = identity();
    let (server, mut good) = server_under_attack(&net, evil.public()).await?;
    let conn = admitted(&net, evil, server.id).await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn a_flood_of_connections_and_garbage_does_not_take_the_server_down() -> anyhow::Result<()> {
    let net = net();
    let evil = identity();
    let (server, mut good) = server_under_attack(&net, evil.public()).await?;
    for _ in 0..50 {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn an_oversized_or_bomb_frame_never_grows_the_client() -> anyhow::Result<()> {
    let net = net();
    let good = identity();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn frames_with_unknown_bases_never_grow_the_client() -> anyhow::Result<()> {
    let net = net();
    let good = identity();
    let evil_id = evil_server(&net, good.public(), |conn| {
        tokio::spawn(async move {
            ...
            for n in 1..=1000u64 {
                ...
                let Ok(diff) = std::panic::catch_unwind(|| emu.snapshot().diff_from(&base)) else {
                    return;
                };
                let frame = Frame {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn a_bad_admission_byte_is_rejected_not_treated_as_admitted() -> anyhow::Result<()> {
    let net = net();
    let good = identity();
